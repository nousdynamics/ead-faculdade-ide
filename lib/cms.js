...
import { readSupabaseCollection, writeSupabaseCollection } from "./supabase/cms-storage.js";
...
export async function readCollection(name) {
  ...
  if (name === "testimonials") {
    if (getStorageProvider() === "supabase") {
      const fromSupabase = await readSupabaseCollection("testimonials");
  ...
  if (getStorageProvider() === "supabase") {
    const fromSupabase = await readSupabaseCollection(name);
    if (fromSupabase !== null) {
      if (name === "testimonial-templates") {
        return normalizeTestimonialTemplates(fromSupabase);
      }
      return fromSupabase;
    }
  }
...
export async function writeCollection(name, data) {
  ...
  if (getStorageProvider() === "supabase") {
    await writeSupabaseCollection(name, payload);
  } else {
  ...
  if (name === "courses") {
    ...
    const courses = Array.isArray(payload) ? payload : all.courses;
    return publishCoursePages(courses, {
      courses,
