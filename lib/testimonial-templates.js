...
function buildAdaptiveBlocks(dep, base, wrap, classeRaiz = DEFAULT_CLASS_ROOT) {
  ...
  const bloco_video = embed
    ...
    : fileVideo
      ? `<div class="${bemClass(classeRaiz, "video")}"><video src="${fileVideo}" title="Depoimento — ${nome}" controls preload="metadata" playsinline style="width:100%;height:100%;display:block;border:0;border-radius:inherit;object-fit:cover;background:#000"></video></div>`
      : "";
