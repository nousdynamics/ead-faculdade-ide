...
const SOCIAL_LINKS = [
  {
    id: "instagram",
    href: "https://www.instagram.com/faculdadeide",
    label: "Instagram",
  },
  {
    id: "facebook",
    href: "https://www.facebook.com/faculdadeide",
    label: "Facebook",
  },
  {
    id: "x",
    href: "https://twitter.com/faculdadeide",
    label: "X (Twitter)",
  },
];

export function socialBrandIcon(name) {
  const path = BRAND_PATHS[name];
  if (!path) return "";

  return `<svg class="social__icon" viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path d="${path}"/></svg>`;
}

export function renderFooterSocialLinks() {
  return `<div class="social">${SOCIAL_LINKS.map(
    ({ id, href, label }) =>
      `<a href="${href}" target="_blank" rel="noopener noreferrer" aria-label="${label}">${socialBrandIcon(id)}</a>`
  ).join("")}</div>`;
}
