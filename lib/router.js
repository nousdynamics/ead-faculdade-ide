...
import { handleAuthPageRequest, handlePublicConfigRequest } from "./user-auth-pages.js";
import {
  isSuperAdminEmail,
  listSiteUsers,
...
export async function routeRequest(req, res, segments) {
  ...
  try {
    if (a === "config" && b === "public" && !c) return handlePublicConfigRequest(req, res);
    if (a === "auth-page" && b && !c) return handleAuthPageRequest(req, res, b);
