...
function getSecret() {
  const secret = process.env.CMS_SECRET?.trim();
  if (!secret || secret === "change-me-in-production") {
    throw Object.assign(
      new Error("CMS_SECRET não configurado. Defina um segredo forte e único para assinar as sessões."),
      { status: 503 },
    );
  }
  return secret;
...
export function verifyToken(token) {
  ...
  try {
    ...
    return null; // sem segredo configurado → ninguém é autenticado (falha fechada)
  }
