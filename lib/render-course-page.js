...
import { parseRdEmbedCode } from "./rd-form-embed.js";
...
function renderCourseFormModals(course) {
  ...
  return [
    ...
    renderRdFormModal({
      ...
      subtitle: forms.investimento_subtitulo || "Preencha o formulário e fale com nossa equipe.",
...
function renderInvestmentSection(inv, inscricaoLink, base, formularios = {}) {
  ...
  const investmentForm = parseRdEmbedCode(formularios.investimento_embed);
  const useInvestmentModal = Boolean(investmentForm.formId);
  const ctaText = escapeHtml(inv.texto_botao || "Adquira");
  ...
  const pricingCta = useInvestmentModal
    ? `<button type="button" class="course-investment__cta course-investment__cta--pricing hovers" data-open-investment-modal>
        <span class="course-investment__cta-icon">${BTN_ICON}</span>
        <span class="course-investment__cta-text">${ctaText}</span>
      </button>`
    : "";

  return `<section class="course-investment${useInvestmentModal ? " course-investment--rd-modal" : ""}" id="investimento" style="--course-investment-check-icon:url('${CHECK_BULLET_URL}')">
