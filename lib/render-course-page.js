 ...
 */
import { renderTestimonialsSection, directVideoUrl } from "./testimonial-templates.js";
import {
  renderFooter,
  renderGtmBody,
  renderGtmHead,
...
// Assets locais — o host WordPress antigo (wp-content/uploads) está fora do ar.
const CHECK_BULLET_URL = "/assets/img/icon-check-bullet.svg";
const ARROW_BULLET_URL = "/assets/img/icon-arrow-bullet.svg";
const SELO_VD_URL = "/assets/img/SELO-VD-IDE-V2.svg";
...
const GUIDE_FEATURES = [
  {
    icon: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/></svg>`,
    text: "Informações completas sobre o curso",
  },
  {
    icon: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>`,
    text: "Conteúdo programático e diferenciais",
  },
  {
    icon: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="m16 11 2 2 4-4"/></svg>`,
    text: "Mercado de trabalho e oportunidades",
  },
];

const GUIDE_LOCK_ICON = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>`;
...
function renderGuideSection(course, base) {
  ...
  // Campos em caixa branca com label flutuante (CSS) — layout da referência.
  const formCard = guide.formId
    ? `<div class="course-guide__form" data-rd-inline-form data-rd-form-id="${escapeHtml(guide.formId)}">
          ${guide.mountHtml}
        </div>
        <p class="course-guide__privacy">${GUIDE_LOCK_ICON}<span>Seus dados estão protegidos. Não enviamos spam.</span></p>`
    : `<button
          type="button"
  ...
  const features = GUIDE_FEATURES
    .map((f) => `<li>${f.icon}<span>${f.text}</span></li>`)
    .join("\n          ");
  ...
  return `<section class="course-guide" id="guia-do-curso">
    <div class="container course-guide__inner${guide.formId ? " course-guide__inner--form" : ""}">
      <div class="course-guide__text">
        <p class="course-guide__eyebrow">Guia do curso</p>
        <h2>Baixe o guia do seu curso e entenda tudo sobre ele</h2>
        <ul class="course-guide__features">
          ${features}
        </ul>
      </div>
      <div class="course-guide__panel">
        ${formCard}
      </div>
...
function renderInvestmentSection(inv, inscricaoLink, base, formularios = {}) {
  ...
  // Só exibe o link de descontos quando o curso tem PDF próprio. URLs de
  // wp-content apontam p/ o WordPress antigo (fora do ar) — tratadas como vazias.
  const pdfRaw = String(inv.pdf_descontos || "").trim();
  const pdfDescontos = /wp-content/i.test(pdfRaw) ? "" : pdfRaw;
  const pdfHref = escapeHtml(assetUrl(pdfDescontos, base));
  const listHtml = beneficios
    .map((b) => `<li>${escapeHtml(b)}</li>`)
    .join("");

  return `<section class="course-investment" id="investimento" style="--course-investment-check-icon:url('${ARROW_BULLET_URL}')">
...
export function renderCoursePage(course, ctx) {
  ...
  // Aceita YouTube ou link direto (upload nativo da aba Vídeos do admin).
  const courseVideoUrl = info.video?.trim() || String(siteConfig.institutionalVideo || "").trim();
  const embed = youtubeEmbed(courseVideoUrl);
  const fileVideo = embed ? "" : directVideoUrl(courseVideoUrl);
  ...
  const infobar = [
    ...
    info.aulas && {
      icon: `${base}assets/img/SolarPlayStreamBold.svg`,
      label: info.aulas,
      wide: false,
    },
    { icon: `${base}assets/img/MaterialSymbolsTouchTripleRounded.svg`, label: "Plataforma interativa", wide: false },
    { icon: `${base}assets/img/e-mec.webp`, label: "Reconhecido pelo e-MEC", wide: true },
  ].filter(Boolean);
        ...
        <div class="e-con-inner">
          <div class="elementor-element elementor-element-fb22a49 e-con-full e-flex e-con e-child">
            ...
            ${
              ...
              <div class="elementor-widget-container">
                <div class="elementor-wrapper elementor-open-inline">
                  <video class="elementor-video" src="${escapeHtml(fileVideo)}" title="Vídeo promocional — ${escapeHtml(course.titulo)}" controls preload="metadata" playsinline style="width:100%;aspect-ratio:16/9;border:0;border-radius:32px;display:block;background:#000;object-fit:cover;"></video>
                </div>
              </div>
            </div>`
                  : ""
