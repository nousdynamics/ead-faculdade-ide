/**
 * Gera páginas estáticas de pós-graduação a partir do CMS (data/cms/*.json).
 * Layout baseado no template Elementor: data/templates/pos-graduacao.elementor.json
 *
...
const DEFAULT_HERO_BG =
  "https://ead.faculdadeide.edu.br/wp-content/uploads/2024/08/ENSINO-DO-BALE-CLASSICO-01.webp";
...
function renderCoursePage(course, ctx) {
  const base = "../../";
  ...
  const heroBg = absUrl(course.imagem_capa) || DEFAULT_HERO_BG;
  ...
  const videoSection = embed
    ? `<section class="course-video"><div class="container"><div class="course-video__frame"><iframe src="${embed}" title="Vídeo promocional — ${escapeHtml(course.titulo)}" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen loading="lazy"></iframe></div></div></section>`
    : "";
...
<body>
  ${renderHeader(base)}
  <main>
    <!-- Template Elementor: hero banner -->
    <section class="course-hero-banner" style="--hero-bg: url('${escapeHtml(heroBg)}')">
      <div class="container course-hero-banner__inner">
        <span class="course-hero__tag">Pós-Graduação</span>
        <h1 class="course-hero-banner__title">${escapeHtml(course.titulo)}</h1>
        ${course.subtitulo ? `<p class="course-hero-banner__subtitle">${escapeHtml(course.subtitulo)}</p>` : ""}
        <div class="course-hero-banner__actions">
          <a class="btn btn--orange" href="${escapeHtml(inscricaoLink)}" target="_blank" rel="noopener">Matricule-se ${ARROW_SVG}</a>
          <a class="btn btn--ghost-light" href="#investimento">Tire dúvidas</a>
        </div>
      </div>
    </section>

    <!-- Template Elementor: barra de conversão -->
    <section class="course-sticky-bar">
      <div class="container course-sticky-bar__inner">
        <div class="course-sticky-bar__info">
          <span class="course-sticky-bar__label">Pós-Graduação</span>
          <strong class="course-sticky-bar__title">${escapeHtml(course.titulo)}</strong>
          ${info.inicio_previsto ? `<span class="course-sticky-bar__meta">Início previsto: ${escapeHtml(info.inicio_previsto)}</span>` : ""}
        </div>
        <div class="course-sticky-bar__actions">
          <a class="btn btn--orange btn--sm" href="${escapeHtml(inscricaoLink)}" target="_blank" rel="noopener">INSCREVA-SE</a>
          <a class="btn btn--outline-light btn--sm" href="https://wa.me/558134650002" target="_blank" rel="noopener">Fale com um consultor</a>
        </div>
      </div>
    </section>
    ...
    <section class="course-hero">
      <div class="container course-hero__inner">
        <div>
          <span class="course-hero__status" style="--status-color:${escapeHtml(statusColor)}">${escapeHtml(statusLabel)}</span>
          ${info.inicio_previsto ? `<p class="course-hero__meta">Início previsto: ${escapeHtml(info.inicio_previsto)}</p>` : ""}
        ...
        <div class="course-hero__media">
          <span class="course-hero__badge">${escapeHtml(statusLabel)}</span>
          <img src="${coverImg}" alt="${escapeHtml(course.titulo)}">
        </div>
      </div>
    </section>
    ...
    <section class="course-guide">
      <div class="container course-guide__inner">
        <div class="course-guide__text">
          ...
          <p class="course-guide__desc">Como funcionam as aulas e atividades avaliativas · Ementa das disciplinas · Ambiente Virtual de Aprendizagem · Perguntas frequentes</p>
          <a class="btn btn--white" href="#investimento">Baixe o guia do curso ${ARROW_SVG}</a>
        </div>
        <div class="course-guide__media">
          <img src="${assetUrl(GUIDE_IMG, base)}" alt="Guia do curso" loading="lazy">
        </div>
      </div>
    </section>
...
async function main() {
  ...
  let selected = courses.filter((c) => c.nivel_formacao_id === "pos-graduacao" || !c.nivel_formacao_id);

  if (args.slug) {
    selected = selected.filter((c) => c.slug === args.slug || c.id === args.slug);
  } else if (args.all !== true && !args.slug) {
  ...
  for (const course of selected) {
    ...
    const outDir = join(ROOT, "pos-graduacao", slug);
    const outFile = join(outDir, "index.html");
    await mkdir(outDir, { recursive: true });
    await writeFile(outFile, renderCoursePage(course, ctx), "utf8");
    console.log(`✓ pos-graduacao/${slug}/index.html`);
  }

  console.log(`\n${selected.length} página(s) gerada(s) a partir do template de pós-graduação.`);
}
